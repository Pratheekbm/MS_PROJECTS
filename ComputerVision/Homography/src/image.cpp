...
Image::Image(char * fileName) {
    if ( NULL == fileName) {
        ...
        if (3 == len[0]) {
            int *local_len;
            local_len = ifssiz(temp);
            local_len[0] = 2;
            local_len[1] = col;
            local_len[2] = row;
            IFSIMG temp1 = ifscreate((char *) "double", local_len, IFS_CR_ALL, 0);
            for (int k = 0; k < 3; k++) {
                temp1 = ifsslice(temp, (char *) "f", k);
                for (int x = 0; x < row; x++) {
                    for (int y = 0; y < col; y++) {
                        img[x][y] += ifsfgp(temp1, x, y) * ifsfgp(temp1, x, y);
                    }
                }
            }
            dimension = 2;
...
imgerr Image::convl(float **kernel, int kernelX, int kernelY) {
    ...
    for (int x = 0; x < row; x++) {
        img[x][0] = tempImage[x][1];
        img[x][col - 1] = tempImage[x][col - 2];
    }

    for (int y = boundryY; y < col - boundryY; y++) {
        img[0][y] = tempImage[1][y];
        img[row - 1][y] = tempImage[row - 2][y];
    }
 ...
 *
 *********************************************************************************************************************/
imgerr Image::div(Image divisor) {

    for (int x = 0; x < row; x++) {
        for (int y = 0; y < col; y++) {
            if (0 != divisor.img[x][y]) {
                img[x][y] = img[x][y] / divisor.img[x][y];
            } else {
                img[x][y] = img[x][y] / 0.01;
            }
        }
    }
    return SUCC;
 ...
 * IN           : input Image, rotationInRad
 * OUT          : None
 * PROCESS      : Calculates to the power of input parameter
 *
 *********************************************************************************************************************/
imgerr Image::power(double inputPower) {

    for (int x = 0; x < row; x++) {
        for (int y = 0; y < col; y++) {
            img[x][y] = pow(img[x][y], inputPower);
        }
    }
