...
Cache* cache_factory::create_cache(int protocol_no, int size, int assoc, int block_size, int num_processors,
        ulong directory_size, int processor_num) {
    switch ((protocol_name) protocol_no) {
    case MESI_dsm:
        return new MESI::MESI(size, assoc, block_size, num_processors, processor_num);
    default:
        return NULL;
    }
}
...
void Cache::print_stats(int processor_number, int protocol, int dir_type) {
    ...
    printf("%.6lf",
            ((double) get_read_misses() + (double) get_write_misses())
                    / ((double) get_reads() + (double) get_writes()));
    cout << endl;
